...
<html lang="zh-CN">
	<!-- 
		请求乱码：
			tomcat7之前：
				get请求乱码： 服务器解析浏览器提交的请求报文的请求首行的参数出现的乱码问题
							get请求由于只能上传请求参数文本，所以可以统一设置编码: 在tomcat server.xml 8080端口号所在的标签添加URIEncoding="UTF-8"
			tomcat8之后：会自动解决get请求乱码
			
			
			post请求乱码： 服务器解析浏览器提交的请求报文的请求体时出现的乱码问题
					post方式可以上传文本参数也可以上传文件，所以不能统一设置编码
					- 在需要处理编码的servlet中  使用request.setCharacterEncoding("UTF-8");	
						springmvc: 使用CharacterEncodingFilter解决
		响应乱码：
			浏览器解析响应报文的响应体时使用的编码格式如何选择？
				1、响应头中的 content-type
				2、如果响应头中没有设置编码，再去响应体中查找meta标签中的编码设置 <meta charset="UTF-8">
				3、如果没有响应体页面也没有设置meta编码，则使用浏览器所在的操作系统默认的编码
	 -->
  <head>
    <meta charset="UTF-8">
    ...
    <!-- 绝对路径是否添加项目名由浏览器解析还是服务器解析决定：
    		jsp语法 属于后台语法：由tomcat服务器解析，绝对路径不需要添加项目名
    			转发、jsp动作标签
    		html语法属于 前端语法： 由浏览器解析 ， 需要添加项目名
    			重定向、html标签的src、href属性、js代码中的路径
     -->
